...
(async function () {
  ...
  function renderHero(data) {
    ...
    return '<section class="hero container" id="hero">\n' +
      ...
      '    <div class="hero__cta">\n' +
      '      <a href="' + data.cta.href + '" class="btn-primary">' + data.cta.text + '</a>\n' +
  ...
  function renderProducts(data) {
    var itemsHtml = data.items.map(function (p) {
      var stackHtml = p.stack.map(function (s) {
        return '<span class="product-card__tag">' + s + '</span>';
      }).join('');

      var linkHtml = '';
      if (p.link) {
        linkHtml = '\n        <a href="' + p.link + '" target="_blank" rel="noopener" class="product-card__link">' + p.link_text + ' <i data-lucide="arrow-up-right"></i></a>';
      }

      return '      <div class="product-card">\n' +
        '        <div class="product-card__icon"><i data-lucide="' + p.icon + '"></i></div>\n' +
        '        <div class="product-card__name">' + p.name + '</div>\n' +
        '        <p class="product-card__tagline">' + p.tagline + '</p>\n' +
        '        <div class="product-card__stack">\n' +
        '          ' + stackHtml + '\n' +
        '        </div>\n' +
        '        <p class="product-card__highlight">' + p.highlight + '</p>' +
        linkHtml + '\n' +
        '      </div>';
    }).join('\n');
    ...
    return '<section class="products container" id="products" style="padding-top: var(--section-gap);">\n' +
      ...
      '    <h2>' + data.title + '</h2>\n' +
      '  </div>\n' +
      '  <div class="products__grid fade-in">\n' +
      itemsHtml + '\n' +
      '  </div>\n' +
  ...
  function renderTechStack(data) {
    var tabsHtml = data.categories.map(function (c, i) {
      var activeClass = i === 0 ? ' active' : '';
      return '<button class="stack__tab' + activeClass + '" role="tab" data-tab="' + c.id + '">' + c.name + '</button>';
    }).join('');

    var panelsHtml = data.categories.map(function (c, i) {
      var activeClass = i === 0 ? ' active' : '';
      var techHtml = c.technologies.map(function (t) {
        return '<span class="stack__tech-pill">' + t + '</span>';
      }).join('');

      return '      <div class="stack__panel' + activeClass + '" id="panel-' + c.id + '" role="tabpanel">\n' +
        '        <div class="stack__technologies">\n' +
        '          ' + techHtml + '\n' +
        '        </div>\n' +
        '        <p class="stack__rationale">' + c.rationale + '</p>\n' +
        '        <p class="stack__proof">' + c.proof + '</p>\n' +
        '      </div>';
    }).join('\n');
    ...
    return '<section class="stack container" id="tech-stack" style="padding-top: var(--section-gap);">\n' +
      ...
      '  </div>\n' +
      '  <div class="fade-in">\n' +
      '    <div class="stack__tabs" role="tablist">\n' +
      '      ' + tabsHtml + '\n' +
      '    </div>\n' +
      panelsHtml + '\n' +
      '  </div>\n' +
  ...
  if (footerRoot) {
    footerRoot.innerHTML = renderFooter(content.footer, content.navbar);
  }
