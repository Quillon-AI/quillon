/* ========================================
    ...
    window.initApp = function () {
      ...
      initHeroMorphText();
      initDataFlow();
      initHeroMatrixTicker();
      initProductScrollStack();
      initCasesCounter();
      initStackDiagram();
      initCiTicker();
    };
    ...
    /* ========================================
       HERO BUILD MATRIX — staggered ticker reveal
       ======================================== */
    function initHeroMatrixTicker() {
      var rows = document.querySelectorAll('.hero__matrix-row');
      ...
      // Stagger reveal, 900ms between rows (as per spec)
      // For a snappier initial load we kick off a bit earlier per-row, 120ms base stagger,
      // then continue the ticker feel with 900ms loops is overkill — use 120ms stagger.
    ...
    /* ========================================
       PRODUCTS — scroll-stack active highlight
       ======================================== */
    function initProductScrollStack() {
      var items = document.querySelectorAll('.product-stack-item');
    ...
    /* ========================================
       CASES — counter-up for numeric metrics
       ======================================== */
    function initCasesCounter() {
      ...
      var observer = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          ...
          var metricEl = entry.target.querySelector('.case-card__metric');
          if (metricEl && !metricEl.dataset.counted) {
            metricEl.dataset.counted = '1';
            animateMetric(metricEl, reduced);
          }
          observer.unobserve(entry.target);
        });
      }, { threshold: 0.3 });
    ...
    function animateMetric(el, reduced) {
    ...
    /* ========================================
       STACK DIAGRAM — tech box hover tooltip / mobile accordion
       ======================================== */
    function initStackDiagram() {
      var svg = document.querySelector('.stack__diagram');
      if (!svg) return;

      var tooltip = document.querySelector('.stack__tooltip');
      if (!tooltip) return;

      var boxes = svg.querySelectorAll('.stack__tech-box');
      boxes.forEach(function (box) {
        box.addEventListener('mouseenter', function (e) {
          var name = box.getAttribute('data-tech');
          var layer = box.getAttribute('data-layer');
          tooltip.innerHTML = '<strong>' + name + '</strong><br><span>' + layer + '</span>';
          tooltip.classList.add('is-visible');
        });
        box.addEventListener('mousemove', function (e) {
          var parent = svg.parentElement.getBoundingClientRect();
          tooltip.style.left = (e.clientX - parent.left + 14) + 'px';
          tooltip.style.top = (e.clientY - parent.top + 14) + 'px';
        });
    ...
    /* ========================================
       CI TICKER — seamless scroll
       ======================================== */
    function initCiTicker() {
      var root = document.getElementById('ci-ticker-root');
      if (!root) return;
      // Render handled by renderer; nothing dynamic needed — CSS handles scroll.
